...
async def ensure_owner_record(db: AsyncSession) -> str:
    ...
    if current_owner and desired and current_owner.wa_phone != desired:
        ...
        if holder is not None and holder.id != current_owner.id:
            ...
            await db.execute(
                update(User).where(User.wa_phone != current_owner.wa_phone).values(is_owner=False)
            )
            await db.commit()
            return current_owner.wa_phone
        current_owner.wa_phone = desired
    ...
    if not current_owner:
        ...
        if holder is not None:
            ...
            db.add(User(wa_phone=desired, is_owner=True, has_permission=True,
                        display_name="You (Owner)"))
            logger.info("Bootstrap: created owner user %s", desired)
        settings.OWNER_WA_PHONE = desired

    await db.execute(
        update(User).where(User.wa_phone != settings.OWNER_WA_PHONE).values(is_owner=False)
    )
    await db.commit()
